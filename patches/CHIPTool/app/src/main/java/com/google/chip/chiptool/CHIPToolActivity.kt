...
class CHIPToolActivity :
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    if (savedInstanceState == null) {
      val fragment = IoBrokerCompanionFragment.newInstance()
    ...
    if (networkType == null) {
      ...
      if (deviceInfo.ipAddress != null) {
        ...
        if (networkType == ProvisionNetworkType.THREAD) {
          val prefs = getSharedPreferences("iobroker_prefs", Context.MODE_PRIVATE)
          ...
          if (channel != -1 && panIdHex != null && extPanId != null && networkKey != null) {
            try {
              ...
              val creds = NetworkCredentialsParcelable.forThread(
                NetworkCredentialsParcelable.ThreadCredentials(opDataset)
              )
              showFragment(DeviceProvisioningFragment.newInstance(deviceInfo!!, creds))
              return
            } catch (e: Exception) {
              Log.e("Companion", "Failed to build pre-fetched operational dataset", e)
            }
          }
        }
